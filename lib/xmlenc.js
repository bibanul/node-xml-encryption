...
exports.encrypt = function(content, options, callback) {
    ...
    if (!content)
        callback(new Error('must provide content to encrypt'));
    if (!options.rsa_pub)
        callback(new Error('rsa_pub option is mandatory and you should provide a valid RSA public key'));
    if (!options.pem)
        callback(new Error('pem option is mandatory and you should provide a valid x509 certificate encoded as PEM'));
    ...
    async.waterfall([
        ...
        function encrypt_content(symmetricKey, cb) {
            switch (options.encryptionAlgorithm) {
                case 'http://www.w3.org/2001/04/xmlenc#aes-256-cbc':
...
exports.decrypt = function(xml, options, callback) {
    ...
    switch (keyEncryptionAlgorighm) {
        ...
        case 'http://www.w3.org/2001/04/xmlenc#rsa-1_5':
            ...
            symmetricKey = pk.decrypt(key,undefined,undefined, ursa.RSA_PKCS1_PADDING);
            break;
        default:
    ...
    switch (encryptionAlgorithm) {
        ...
        case 'http://www.w3.org/2001/04/xmlenc#aes128-cbc':
            ...
            var decipher = crypto.createDecipheriv(algo, symmetricKey, encrypted.slice(0, 16));
            if (encrypted.length % 16 == 0) decipher.setAutoPadding(false); //if we don't, the final() will throw bad decrypt. Also this works only if data length is multiple of cipher block size (16)
...
function encrypt_aes128cbc(symmetricKey, content, encoding, callback) {
    // create a random iv for aes-128-cbc
    crypto.randomBytes(16, function(err, iv) {
        if (err) return callback(err);

        var cipher = crypto.createCipheriv('aes-128-cbc', symmetricKey, iv);
        // encrypted content
        var encrypted = cipher.update(content, encoding, 'binary') + cipher.final();
        return callback(null, Buffer.concat([iv, new Buffer(encrypted, 'binary')]));
    });
